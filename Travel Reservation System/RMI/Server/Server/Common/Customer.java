...
public class Customer extends RMItem
{
	...
	public String generate_summary(){
		String s = "Customer ID: " + this.m_ID + '\n';
		for (String key : m_reservations.keySet()){
			ReservedItem item = (ReservedItem) m_reservations.get(key);
			s += + item.getCount() + " item(s) of " + item.getReservableItemKey() + " purchased"  + ", with a total price of $" + item.getPrice()*item.getCount() + "\n";
		}
